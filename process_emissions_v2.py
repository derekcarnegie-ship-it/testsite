...
<div id="page">
  ...
  <div id="text-panel">
    ...
    <div class="source-notes">Note: Emissions exclude land-use change and forestry. World Bank country income groups definitions are from FY26. Data are plotted on a log-log scale.<br>Source: EDGAR, World Bank.</div>
...
function highlight(html) {
  document.getElementById('highlight-box').innerHTML = html;
}
...
function phase1() {
  ...
  function frame(now) {
    ...
    if (t >= 23) {
      ...
      highlight('');
      // 1s pause, then fade tips and lines to grey; ph1Start stays blue for phase2
      setTimeout(() => {
        ph1Tips.selectAll('circle').transition().duration(600).attr('opacity', 0);
...
function phase2() {
  ...
  highlight('<div style="font-size:34px;letter-spacing:1px;line-height:1.05">RELATIVE<br>DECOUPLING</div>');
  paths.attr('d', d => d.pathData[23]);

  // Cross-fade: ph1Start (blue seed dots) fade out as dots2000g fade in
  if (ph1Dots) {
    ph1Dots.transition().duration(700).attr('opacity', 0)
      .on('end', () => { if (ph1Dots) { ph1Dots.remove(); ph1Dots = null; } });
  }
...
function phase4() {
  ...
  setTimeout(() => {
    ...
    highlight('<div style="font-size:34px;letter-spacing:1px;line-height:1.05">INCOME &#x2B06;<br>EMISSIONS &#x2B07;</div>');
...
function phase5() {
  ...
  highlight('<span style="font-size:74px;letter-spacing:-1px">8.7\u00d7</span>');
  ...
  // Loop: fade everything out then restart (~0.5s blank before restart)
  setTimeout(() => {
